...
def trains_interface
  loop do
    ...
    puts '2: Manage trains' unless @trains.empty?
    ...
    when 2
      show_trains
      puts "1-#{@trains.count}: Choose train"
      puts '0: Back to navigation'

      input = validate_input(gets.chomp, (0..@trains.count))

      manage_train(@trains[input - 1])
    else
      break
    end
  end
...
def manage_train(train)
  loop do
    ...
    puts '2: Take route' unless @routes.empty?
    ...
    when 2
      ...
      puts "1-#{@routes.count}: Choose route to set for train"
      ...
      train.take_route(@routes[input - 1])
    when 3
      move_train(train)
    else
      break
